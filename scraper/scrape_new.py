...
TOKEN = "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJwcm9maWxlSUQiOiJkYWUyZDZkMWUwMGY0OWE2YTA5MTIyNGUzZDA4ZGVhYyIsInN1YiI6InNaTzF3ejNaWm5KbmZhMllCeS9XN21QQjJLelF1SUp2WGVab1plV3Ywc0U9IiwiaWF0IjoxNzc0NTYxOTg1MDU1LCJleHAiOjE3NzQ1NjI4ODUwNTV9.DCOYB-sMWgV6_HimSdw-Ib9ykU9SPJwtdCDZpEQgTvL04w8nBtGhGui0qzqjxoO0_UgsMY1zyxCIcNDm3-upy_yDTxIaHMX1y6POsydw-9oe3CBUtBgYRlI9_9iF5pOXCI8-2Bd3fMcw25Ri9NMncHJ3YlC74oOvyCgm5p_3IRYPkrLapKBq1adYDwCk5b_UE3hTWZVJOj_maqromK2yW2Ajhy3JJ6OqgrzjL1THLxSz9BeYgtUoKKDosz8sKd-jo4Iv2vPBUG-CBfZCo6kwOXU-5uFuabAkkUbzMtUZT301wiMh8mGgaX5ifqmHV9OMIk7SQFULIHvbPj44snBlHw"
...
SEARCH_URL = "https://shop.mercedes-benz.co.uk/api/v4/vehicles/search/new/national"
PROGRESS_FILE = "data/new/progress.json"
CHUNKS_DIR = "data/new/chunks"
FINAL_FILE = "data/new/mercedes_new_cars_FULL.json"
CREDS_FILE = "data/new/credentials.json"
...
def make_payload(page):
    return {
        "Criteria": {
            ...
            "ExcludeUnderOfferVehicles": True
        },
        "Sort": {"Id": 1},
        "Finance": {"Criteria": {
...
def get_start_page():
    if os.path.exists(PROGRESS_FILE):
        ...
        print(f"🔄 Resuming from page {start}/{total}")
        return start, data.get("total_pages")
    print("🆕 Starting fresh — New Cars")
    return 0, None


def save_progress(page, total_pages):
...
def scrape():
    os.makedirs(CHUNKS_DIR, exist_ok=True)
    start_page, total_pages = get_start_page()
    ...
    if total_pages is None:
        ...
        r = requests.post(SEARCH_URL, headers=make_headers(
            current_token), json=make_payload(0))
        if r.status_code == 403:
            print("⚠️  Token expired on first request — attempting auto-refresh...")
            current_token, current_refresh = do_refresh(
                current_token, profile_id, current_refresh)
            if not current_token:
                print(
                    "❌ Could not refresh. Paste a fresh TOKEN into the script and re-run.")
                return
            save_credentials(current_token, profile_id, current_refresh)
            r = requests.post(SEARCH_URL, headers=make_headers(
                current_token), json=make_payload(0))
        results = r.json()["SearchResults"]
        total_pages = results["TotalPages"]
        print(
            f"✅ Found {results['TotalResults']:,} new cars across {total_pages} pages")
        save_chunk(0, results["Vehicles"])
        save_progress(0, total_pages)
        start_page = 1
            ...
            if r.status_code == 403:
                ...
                if new_token:
                    ...
                    if r.status_code != 200:
                        ...
                        save_progress(page - 1, total_pages)
                        break
                else:
                    print(
